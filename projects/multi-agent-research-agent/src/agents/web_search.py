...
web_search_agent = AgentDefinition(
    ...
    prompt=(
        ...
        "3. If the search result includes an `ERROR:` block naming an unavailable "
        "(timed-out) source that is `retryable: true`, USE the partial results shown above "
        "the block (they are valid), and retry the search once or twice (up to 2 — keep in "
        "sync with config.MAX_TOOL_RETRIES) in case the source recovers. If it still fails, "
        "do NOT fabricate its content: pass along the unavailable-source note (its failure "
        "type, the source name, and that it is retryable) so the coordinator can annotate it. "
        "An access failure like this is DISTINCT from a valid empty result (which means the "
        "corpus genuinely has no such source — report that as an empty finding, not a "
        "failure).\n"
        "4. Return a DISTILLED result, not a transcript. Keep it under ~1–2k tokens:\n"
